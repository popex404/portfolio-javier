...
function AnalysisCard({ analysis }) {
  const displayTitle = analysis.titleEN || analysis.title
  const dashIdx = displayTitle.indexOf(' — ')
  const mainTitle = dashIdx !== -1 ? displayTitle.slice(0, dashIdx) : displayTitle
  const subtitle = dashIdx !== -1 ? displayTitle.slice(dashIdx + 3) : null
  ...
  return (
    <div className="bg-card/50 backdrop-blur-sm border border-border rounded-xl p-4 sm:p-6 shadow-xl">
      ...
      <p className="text-xs text-muted-foreground mb-4 -mt-1">
        Snapshots of dashboards developed using FlipsideCrypto. The original reports are no longer available online — these are the visual records that remain.
      </p>
      ...
      <div className="mt-4">
        <p className="text-xs text-muted-foreground/60 italic mt-3 mb-1">Click the image to view it at full size.</p>
        {(() => {
          const displayTitle = current.titleEN || current.title
          const dashIdx = displayTitle.indexOf(' — ')
          const mainTitle = dashIdx !== -1 ? displayTitle.slice(0, dashIdx) : displayTitle
          const subtitle = dashIdx !== -1 ? displayTitle.slice(dashIdx + 3) : null
          return (
...
export default function DashboardsEN() {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12 pb-8 sm:pb-12">
        ...
        <div className="mb-4 animate-fade-in">
          <div className="bg-card/50 backdrop-blur-sm border border-border rounded-xl p-5 sm:p-6">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              Between 2022 and 2024 I worked as an independent analyst using{' '}
              ...
              as my main platform, alongside tools like{' '}
              ...
              and Dune. I built dashboards and reports on capital flows, DeFi and NFT activity, and market events across Solana, Avalanche, Ethereum, NEAR, and nine more networks.
              I was recognized as <span className="text-foreground font-medium">Top Analyst at Flipside Crypto</span> and <span className="text-foreground font-medium">Data Alchemist at CovalentHQ</span>.
            </p>
          </div>
        </div>
        ...
        <div className="mb-8 animate-fade-in">
          <div className="bg-card/50 backdrop-blur-sm border border-border rounded-xl p-5 sm:p-6">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              Every analysis was shared on Twitter: threads explaining the key findings to crypto communities.
              Over time, that generated recognition from projects and left a public record of four years of work.
              In 2025, Flipside migrated to an AI model and the platform ceased to exist. What remains are these tweets:{' '}
              <span className="text-foreground font-medium">40+ analyses</span> across{' '}
              <span className="text-foreground font-medium">13 blockchain networks</span>.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Twitter posts explaining the key highlights of my dashboards to crypto communities, through interactive threads.
          </p>

          {/* Primary chain buttons */}
          <div role="group" aria-label="Filter by blockchain" className="flex flex-wrap gap-2 mb-2">
          ...
          {(showOthers || isSecondaryActive) && (
            <div role="group" aria-label="Filter by blockchain" className="flex flex-wrap gap-2 mb-2 mt-2 pl-3 border-l-2 border-border">
          ...
          <div className="space-y-2 mt-5">
            {filtered.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-8">
                No analyses for this blockchain.
              </p>
            )}
        ...
        <footer className="mt-8 pt-6 border-t border-border text-center">
          <p className="text-xs text-muted-foreground">
            Made by Javier García ·{' '}
            <Link to="/en" className="text-primary hover:underline">Back to portfolio</Link>
          </p>
        </footer>
      </div>
    </div>
